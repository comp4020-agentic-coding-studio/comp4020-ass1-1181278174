...
const MIN_TRIPS = 65;
const SEEDS = [1, 2, 3];
const RHOS = [0, 0.0002, 0.0005, 0.001, 0.002, 0.005, 0.01];
const HEALED_AT = 1.6;
...
function run(rho: number, seed: number, wantMaps: boolean): Result {
  ...
  for (let s = 1; s <= AFTER; s += 1) {
    ...
    if (s === 1000) at2000 = ratio;
    if (s === 3000) at6000 = ratio;
...
function main(): void {
  ...
  say();
  say(`| ρ | before (÷${BFS_OPEN}) | trips before | trapped on bar | 65 post-block trips by | reading +1000 | +3000 | +${AFTER} | healed (≤${HEALED_AT}×) at | post-block trips |`);
  say(`|---|---|---|---|---|---|---|---|---|---|`);
  ...
  writeFileSync("docs/spikes/2026-08-17-field-v2-block-fine-advisor.md", `${out.join("\n")}\n`);
}
main();
